...
class TrainingThread(QtCore.QThread):
    ...
    def run(self):
        ...
        train_loader = load_data(train_path, batch_size=netBatchSize)  #load data
        ...
        criterion = nn.BCELoss()                                                #initialize network, optimizer and loss function
        now_time = datetime.now()
        time_str = datetime.strftime(now_time, '%m-%d_%H-%M-%S')
        log_dir = os.path.join('./Result/', time_str)
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        writer = SummaryWriter(log_dir)                                           #use tensorboardX to record information
        ...
        noise_test = torch.Tensor(noise_test).to(device)                    # create noise input for generator

        for epoch in range(netMaxEpoch):
            global now_epoch
            now_epoch = epoch+1             #epoch
            for i, data in enumerate(train_loader, 0):      #train Discriminator
            ...
            for i in range(int(60000 / netBatchSize)): #train Generator
            ...
            with torch.no_grad():
                ...
                imageio.imwrite('./Result/show4', show4, '.png')            #save some results for showing
...
class Ui_MainWindow(object):
    ...
    def settraining(self):
        ...
        if self.Statuslabel.text() == 'Status: Stop' and data_ready==True:
            ...
            self.th = TrainingThread()  # start the training threads
            self.th.start()
    ...
    def updateepoch(self):      #show results
